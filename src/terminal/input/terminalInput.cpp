...
TerminalInput::OutputType TerminalInput::_searchWithModifier(const KEY_EVENT_RECORD& keyEvent)
{
    if (const auto match = _searchKeyMapping(keyEvent, s_modifierKeyMapping))
    {
        ...
        if (!v.sequence.empty())
        {
            const auto shift = WI_IsFlagSet(keyEvent.dwControlKeyState, SHIFT_PRESSED);
...
TerminalInput::OutputType TerminalInput::HandleFocus(const bool focused) const noexcept
{
